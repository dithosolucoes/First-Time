...
const NotebookCard: React.FC<NotebookCardProps> = ({ notebook, onClick, type, onOpenContextMenu }) => {
    
    if (type === 'new') {
        return (
            <button 
                onClick={onClick} 
                className="group bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg p-4 flex flex-col items-center justify-center text-center aspect-[4/3] hover:shadow-md transition-shadow"
            >
                <div className="w-12 h-12 bg-gray-100 group-hover:bg-[var(--accent-light)] rounded-full flex items-center justify-center mb-3 transition-colors">
    ...
    if (type === 'recent') {
        ...
        return (
            <div onClick={onClick} className="group bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg p-4 flex flex-col aspect-[4/3] cursor-pointer hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                    {Icon && <Icon className="text-[var(--text-secondary)]" size={24} />}
                    {onOpenContextMenu &&
                        <button onClick={onOpenContextMenu} className="p-1 -mr-2 -mt-1 text-[var(--text-tertiary)] hover:text-[var(--text-primary)] opacity-0 group-hover:opacity-100 transition-opacity">
